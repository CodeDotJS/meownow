...
test("a live profile wins and is stored", () => {
	...
	expect(resolveSession({ status: 200, profile, cachedMe: cached, hasLocal: true })).toEqual(
		profile,
	);
});

test("offline with keys and a snapshot keeps the tray signed in", () => {
	expect(resolveSession({ status: 0, profile: null, cachedMe: cached, hasLocal: true })).toEqual(
		cached,
	);
});
